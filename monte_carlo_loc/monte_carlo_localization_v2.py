...
"""
To run this program
python3 monte_carlo_localization_v2.py
python3 occupancy_grid_publisher.py
rviz (with config comp417.rviz)
...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        self.weights = exp_normalize(-1 * np.array(errors) / (self.beam_range_measurement_noise_std_dev ** 2))
        self.weights = self.weights / np.sum(self.weights)

        # For debug.
        # idx = self.weights.argmax()
        # dist_vec = np.array([self.particles[idx].x - self.robot_odom.pose.pose.position.x,
        #         self.particles[idx].y - self.robot_odom.pose.pose.position.y])
        # print("most likely particle {} {} {}".format(self.particles[idx].x, self.particles[idx].y, self.particles[idx].theta))
        # print("dist to real position {} ".format(np.linalg.norm(dist_vec)))
    ...
    def simulate_laser_scan_for_particle(self, x, y, yaw_in_map, angles, min_range, max_range):
        ...
        for a in angles:
            ...
            while True:
                ...
                tmp_x = cos(actual_angle_map) * d + x
                tmp_y = sin(actual_angle_map) * d + y
                if tmp_x < self.xmin or tmp_x > self.xmax or tmp_y < self.ymin or tmp_y > self.ymax:
                    out_of_boundary = True
                    break
                (row, col) = self.metric_to_grid_coords(tmp_x, tmp_y)
                if self.grid_bin[row][col]:
                    # Empty
                    pass
                else:
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        simulation_ranges = self.simulate_laser_scan_for_particle(
            particle.x, particle.y, particle.theta, angles_in_upwards_baselaser_frame,
        ...
        norm_error = np.linalg.norm(np.array(actual_ranges) - np.array(simulation_ranges))
        return norm_error**2
