...
2.
python3 occupancy_grid_mapper.py
3. start rviz with the config file
rviz.rviz
"""
...
from math import pi, log, exp, fabs
import random
import numpy as np
import sys
...
class HuskyMapper:
    def __init__(self, num_rows, num_cols, meters_per_cell):
        ...
        self.odometry_position_noise_std_dev = 0 # 0.1
        self.odometry_orientation_noise_std_dev = 0 #2.5
    ...
    def odometry_callback(self, msg):
        ...
        # 
        self.q_map_baselink = np.array([self.odometry.pose.pose.orientation.x,
        ...
        #
        #
        self.p_map_baselink = np.array([self.odometry.pose.pose.position.x,
        ...
        #
        # Note, two ways to get this:
        #   1.
        #   np.dot(self.R_map_baselink, self.R_baselink_baselaser)
        #   2.
        #   self.q_map_baselaser = tr.quaternion_multiply(self.p_map_baselink, self.p_baselink_baselaser)
        ...
        #       multiplication, which is not usually what you want
        #
        self.p_map_baselaser = np.dot(self.R_map_baselink, self.p_baselink_baselaser) + self.p_map_baselink
    ...
    def is_in_field_of_view(self, robot_row, robot_col, robot_theta, row, col):
        ...
        # TODO: fill this
        #
        # Note that x maps to col and y maps to row.
        dist_x = fabs(col - robot_col) * self.ogm.meters_per_cell
        dist_y = fabs(row - robot_row) * self.ogm.meters_per_cell
        dist = sqrt(dist_x * dist_x + dist_y * dist_y)
        if dist < self.min_laser_range or self.max_laser_range < dist:
            return False
        dx = (col - robot_col) * self.ogm.meters_per_cell
        dy = (row - robot_row) * self.ogm.meters_per_cell
        cell_theta = atan2(dy, dx)
        d_theta = atan2(sin(cell_theta - robot_theta), cos(cell_theta - robot_theta))
        if d_theta < self.min_laser_angle or self.max_laser_angle < d_theta:
            return False
        return True
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        dist_x = fabs(col - robot_col) * self.ogm.meters_per_cell
        dist_y = fabs(row - robot_row) * self.ogm.meters_per_cell
        r = sqrt(dist_x * dist_x + dist_y * dist_y)
        dx = (col - robot_col) * self.ogm.meters_per_cell
        dy = (row - robot_row) * self.ogm.meters_per_cell
        cell_theta = atan2(dy, dx)
        diff_angle = atan2(sin(cell_theta - robot_theta_in_map), cos(cell_theta - robot_theta_in_map))
